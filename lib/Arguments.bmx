...
Rem	IMPLEMENTED ARGUMENTS
	...
	-ast			DEPRECIATED, PLEASE USE -o:ast
	-eol			DEPRECIATED, PLEASE USE -o:eol
	-o:<options>
					ast			Show AST in outline
					eol			Show EOL in AST
					noname		Do not display token name in outline
	
	** Options can be combined by seperating them with a comma, for example:
...
'DebugLog( "Setting '"+key+"' to '"+value+"'" )
		...
		If Features.isInvalid() 
			...
			'Observer.port( ERROR, "INTERNAL ERROR" )
			Trace.Error( "TArguments.New() - Invalid Argument Data" )
		Else
'DebugStop
		'DebugLog( "ARGS:"+AppArgs.length )
			...
			For Local n:Int=1 Until AppArgs.length
				...
				If Not Features.contains( section )
					Trace.Error( "Invalid argument: "+AppArgs[n] )
					Continue
				End If
	...
	'DebugLog( "- VALID "+section+":"+keyvalue[0])
					...
					Else
						Trace.Error( "Invalid argument: "+section+":"+key )
					End If

...
DebugStop
		...
		Local typestr:String = "T"+invoke_method
...
DebugStop
		...
		Local cursor:TTypeId = typeid
		Repeat
		
			For Local fld:TField = EachIn cursor.EnumFields()
				Print fld.name()
				fields.addlast( fld.name() )
				
				Local fldtype:TTypeId = TypeIdForTag( fld )
				
				
				'Local f:JToken = New JToken( fld.name()
				
				'fld.set<fieldtype>( root[fld.name()] )
				
			Next
		
			'cursor.EnumFields( fields )
			cursor = cursor.superType()
		Until cursor = Null
		'
		For Local f:String = EachIn fields
			Print "Field: f"
		Next
...
'DebugStop
	...
	Method _Stringify:String( j:JNode )
		...
		Select j.class
		Case "object"
			Local map:TMap = TMap( j.value )
			text :+ "{"
			If map
			 
				For Local key:String = EachIn map.keys()
					text :+ "~q"+key+"~q:"
					text :+ _stringify( JNode( map[key] ) )
					text :+ ","
				Next
		...
		Case "number"
			text :+ String(j.value)
		Case "string"
			text :+ "~q"+String(j.value)+"~q"
		Default
			Print "INVALID SYMBOL: '"+j.class+"'"
		End Select
		Return text
	End Method
